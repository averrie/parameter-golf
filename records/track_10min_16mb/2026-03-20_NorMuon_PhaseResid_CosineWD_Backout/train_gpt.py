"""
Training: RoPE 50K, softcap 20, backout, cosine WD, momentum cooldown,
phase-transition resid_mix, NorMuon, cautious WD, batch 524K (more gradient steps)
Eval: LoRA TTT (causal per-document), sliding window (stride=64)
Compression: int5-MLP/int6-attn, magnitude pruning 5%, zstd-22 (zlib-9 fallback)
"""
...
class Hyperparameters:
    ...
    # Test-time training: LoRA (per-document)
        ...
        for group in self.param_groups:
            ...
            for i, p in enumerate(params):
                if i % world_size == rank and p.grad is not None:
                    ...
                    # NorMuon: per-neuron adaptive scaling via second moment
                    if g.ndim == 2:
                        v_mean = g.float().square().mean(dim=-1, keepdim=True)
                        if "second_moment" not in state:
                            state["second_moment"] = v_mean.clone()
                        else:
                            state["second_moment"].lerp_(v_mean, 1.0 - normuon_beta2)
                        g = g * state["second_moment"].clamp_min(1e-10).rsqrt().to(
                            g.dtype
                        )
            ...
            for p in params:
                ...
                if wd > 0:
                    # Cautious weight decay: only when update and weight agree in sign
                    mask = (g * p.data) >= 0
                    p.data.sub_(lr * wd * p.data * mask.to(p.dtype))
                p.add_(g, alpha=-lr)
                curr += p.numel()
        return loss
...
_NUM_LAYERS_FOR_PATTERNS = int(os.environ.get("NUM_LAYERS", 10))
...
CONTROL_TENSOR_NAME_PATTERNS = tuple(
    ...
    for pattern in os.environ.get(
        ...
        "skip_weight,skip_weights,smear,bigram.scale,backout_lambda",
    ).split(",")
    if pattern
)
FP16_KEEP_NAME_PATTERNS = tuple(
    ...
    for pattern in os.environ.get(
        ...
        f"tok_emb,blocks.{_NUM_LAYERS_FOR_PATTERNS - 1}.attn.c_k",
    ).split(",")
    if pattern
)
...
def quantize_state_dict_int6(state_dict: dict[str, Tensor]):
    ...
    for name, tensor in state_dict.items():
        ...
        if any(pattern in name for pattern in FP16_KEEP_NAME_PATTERNS):
            ...
            continue
        # Mixed quantization: int5 for MLP, int6 for attn/bigram
...
class GPT(nn.Module):
    ...
    def _body(self, input_ids: Tensor, lora=None):
        ...
        for i in range(self.num_decoder_layers):
            ...
            # Capture midpoint (first decoder block output) for backout
            if bi == self.backout_layer and x_backout is None:
                x_backout = x
...
class BatchedTTTLoRA(nn.Module):
    def __init__(self, bsz: int, model: GPT, rank: int):
        ...
        self.q_loras = nn.ModuleList(
            [
                BatchedLinearLoRA(bsz, dim, b.attn.c_q.weight.shape[0], rank)
                for b in model.blocks
            ]
        )
        self.v_loras = nn.ModuleList(
            [
                BatchedLinearLoRA(bsz, dim, b.attn.c_v.weight.shape[0], rank)
                for b in model.blocks
            ]
        )
...
def main() -> None:
    ...
    while True:
        ...
        if last_step or (args.val_loss_every > 0 and step % args.val_loss_every == 0):
            ...
            log0(
                f"step:{step}/{args.iterations} val_loss:{vl:.4f} val_bpb:{vb:.4f} train_time:{training_time_ms:.0f}ms"
            )
            torch.cuda.synchronize()
            t0 = time.perf_counter()
        if last_step:
        ...
        # Muon momentum: warmup then cooldown during warmdown
        frac = (
        ...
        mom = (
            warmup_mom
            if scale >= 1.0
            else warmup_mom + (1.0 - scale) * (0.90 - warmup_mom)
        )
        for group in optimizer_muon.param_groups:
            group["momentum"] = mom
        ...
        if max_wallclock_ms:
            wd_frac = elapsed_ms / max_wallclock_ms
        else:
            wd_frac = step / max(args.iterations, 1)
        cosine_wd = base_wd * 0.5 * (1.0 + math.cos(math.pi * min(wd_frac, 1.0)))
        for opt in optimizers:
            for group in opt.param_groups:
                group["lr"] = group["base_lr"] * scale
                if "weight_decay" in group:
                    group["weight_decay"] = cosine_wd
    ...
    if args.swa_enabled and swa_state and swa_count > 1:
        ...
        csd = base_model.state_dict()
        base_model.load_state_dict(
            {n: (t / swa_count).to(dtype=csd[n].dtype) for n, t in swa_state.items()},
            strict=True,
        )
    ...
    # Magnitude pruning: zero out smallest 5% of large weight matrices
    with torch.no_grad():
    ...
    quant_result, quant_meta = quantize_state_dict_int6(sd_cpu)
    ...
    with open("final_model.int8.ptz", "rb") as f:
        qbd = f.read()
    if _COMPRESSOR == "zstd":
        dec = zstandard.ZstdDecompressor().decompress(qbd)
    else:
        dec = zlib.decompress(qbd)
