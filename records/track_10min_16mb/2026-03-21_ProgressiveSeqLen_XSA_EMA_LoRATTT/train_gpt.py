"""
Progressive Sequence Length + XSA + EMA + LoRA TTT
- Phase 1: train at short seq_len (512) for fast steps and more total tokens
- Phase 2: switch to long seq_len (2048) for long-context quality
- 11 layers, dim=512, 8 heads / 4 KV heads, MLP 3x Squared ReLU
- XSA on last 4 layers, EMA (decay=0.997)
- LoRA TTT eval (causal per-document, adapts Q/K/V and lm_head, eps=1e-4)
- FA3 with SDPA fallback, NTK-aware RoPE
- BigramHash(10240), SmearGate, orthogonal init and μP
- Int6 for block weights, int8 for bigram, FP16 tok_emb, zstd-22
"""
...
class Hyperparameters:
    ...
    # Progressive sequence length: train short first, then switch to long.
    phase1_seq_len = int(os.environ.get("PHASE1_SEQ_LEN", 512))
    phase1_frac = float(os.environ.get("PHASE1_FRAC", 0.67))

    # LoRA TTT eval parameters.
    ...
    def forward(self, x: Tensor, q_delta=None, k_delta=None, v_delta=None) -> Tensor:
        ...
        if _HAS_FA3:
            ...
            if self.use_xsa:
                y_t = y.transpose(1, 2)
                v_t = v.transpose(1, 2)
                y_t = self._xsa_efficient(y_t, v_t)
                y = y_t.transpose(1, 2)
...
class Block(nn.Module):
    ...
    def forward(
        self, x: Tensor, x0: Tensor, q_delta_fn=None, k_delta_fn=None, v_delta_fn=None
    ) -> Tensor:
...
class GPT(nn.Module):
    ...
    def _body(self, input_ids: Tensor, lora=None) -> Tensor:
        """Shared forward body. Returns logits [B, T, V]."""
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor, lora=None) -> Tensor:
        ...
        if lora:
            # For TTT: return per-token losses [B, T]
...
class BatchedTTTLoRA(nn.Module):
    def __init__(self, bsz: int, model: GPT, rank: int):
        ...
        self.q_loras = nn.ModuleList(
            [
                BatchedLinearLoRA(
                    bsz, dim, model.blocks[i].attn.c_q.weight.shape[0], rank
                )
                for i in range(num_layers)
            ]
        )
        self.k_loras = nn.ModuleList(
            [
                BatchedLinearLoRA(
                    bsz, dim, model.blocks[i].attn.c_k.weight.shape[0], rank
                )
                for i in range(num_layers)
            ]
        )
        self.v_loras = nn.ModuleList(
            [
                BatchedLinearLoRA(
                    bsz, dim, model.blocks[i].attn.c_v.weight.shape[0], rank
                )
                for i in range(num_layers)
            ]
        )
...
def main() -> None:
    ...
    enable_flash_sdp(not (sys.platform == "win32"))
    ...
    log0(
        f"progressive_seq_len: phase1_seq_len:{args.phase1_seq_len} phase1_frac:{args.phase1_frac}"
    )
    ...
    # Warmup primes compiled forward/backward paths for BOTH seq_len values, then
    # restores the initial weights/optimizer state so measured training starts from true init.
    if args.warmup_steps > 0:
        ...
        model.train()
        warmup_seq_lens = [args.phase1_seq_len, args.train_seq_len]
        for warmup_step in range(args.warmup_steps):
            seq_len = warmup_seq_lens[warmup_step % len(warmup_seq_lens)]
            zero_grad_all()
            for micro_step in range(grad_accum_steps):
                ...
                x, y = train_loader.next_batch(
                    args.train_batch_tokens, seq_len, grad_accum_steps
                )
                with torch.autocast(
            ...
            if (
                ...
                log0(
                    f"warmup_step:{warmup_step + 1}/{args.warmup_steps} seq_len:{seq_len}"
                )
    ...
    phase1_ms = (
        max_wallclock_ms * args.phase1_frac if max_wallclock_ms is not None else None
    )
    ...
    while True:
        ...
        # Progressive sequence length: short seq phase then long seq phase
        in_phase1 = phase1_ms is not None and elapsed_ms < phase1_ms
        current_seq_len = args.phase1_seq_len if in_phase1 else args.train_seq_len
        if not in_phase1 and not phase_switched:
            phase_switched = True
            log0(
                f"phase_switch: seq_len {args.phase1_seq_len} -> {args.train_seq_len} at step:{step} elapsed:{elapsed_ms:.0f}ms"
            )
    ...
    # Reset dynamo cache before eval
    torch._dynamo.reset()
    if sys.platform == "win32":
